...
test("Canvas stop discards buffered frames immediately", async ({ page }) => {
  await login(page);
  await page.addInitScript(() => {
    const counters = { decoded: 0, drawn: 0, closed: 0 };
    Object.defineProperty(window, "__rdPresentationCounters", { value: counters, configurable: true });
  ...
  await page.goto("/remotedesktop?clientId=canvas-stop-test");

  // Simulate user Stop even if the button is disabled while idle.
  await page.evaluate(() => {
    document.getElementById("stopBtn")?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
  });
  ...
  ).__rdPresentationCounters)).toEqual(before);
});
